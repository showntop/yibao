...
- [ ] 所有 >1000 行的源文件拆分完毕（前端组件、`lib.rs`、`server.py`、`coding.py`、`brain.ts`）。（brain.ts ✅ 8 行；coding.py ✅ 798、HomePlugins.vue ✅ 933；perception.py ✅ 1027→390（R-32c 三分：store/sensors/编排）；App.vue 1711 → R-08 剩余可选项；server.py ✅ 1334 → R-13 已落地：查表化 + 四闭包域拆 `runtime/`，serve_async 剩 956 行为 handler 薄层/组件装配/主循环收尾——暂留为显式决策）
