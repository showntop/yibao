...
| 🟡 部分完成 | **R-12 lib.rs 拆分**：行数 2594→1584（+ commands.rs 914 / snip.rs / plugin_manifest.rs，测试外移）✅；**3.2 分层未建**——sidecar 守护（spawn_brain/spawn_bridge/spawn_watchdog/restart_with_backoff）、配置（env/setup）、窗口/热区/剪贴板系统集成仍在 lib.rs，commands 为单文件非目录。剩余项 → **R-12b** |
...
| 🟡 部分完成 | **R-20 invoke 收敛**：SetupWizard/PanelApp/SnipOverlay 已走 brainClient ✅；残留直调：`App.vue:143` 与 `HomeChat.vue:209` 的 `invoke("get_setup_config")`（brainClient.getSetupConfig 已存在）、`WebviewPanel.vue:104` 动态 invoke、`App.vue:247/470` 与 `Home.vue:394` 窗口命令直调（brainClient 已有 openHomeWindow 等同类封装，风格不一） |
