...
| ✅ 完成 | **R-23/R-24 mobile（2026-08-22，R-34 收口）**：R-23「api/http.ts 请求收敛」✅ +「useEventStream」✅（实质已达成于 api/events.ts）；R-24「StreamLike 唯一化」✅ +「模块级单例 ref → provide/inject」按决策放弃（单窗口 SPA mini-Pinia 形态，R-34 行落档）；**补 Approvals/Chat 竞态守卫**（修真实 EventSource 泄漏，vitest 83 全绿） |
...
| ✅ 完成 | **阶段 6 蓝图收口（2026-08-22 完成）**：R-36 文档补账 ✅、R-35 `_sibling` 归一 ✅（8de7f92）、R-32 sidecar 3.3 实质收口 ✅（a distiller_store a1e2d1b / b llm_vision 5cc612b / c perception 三分 f869cd0，每步 1142 全绿）、R-33 commands.rs ✅ 显式放弃（97e8446）、R-34 mobile 收口 ✅（补竞态守卫 + 决策落档，vitest 83 全绿，见下）。任务详单与放弃清单见第 4 节阶段 6；3.2/3.3/3.4 蓝图执行注记见第 3 节 |
| ✅ 完成 | **R-34 mobile 收口（2026-08-22）**：useEventStream **实质已达成**（`api/events.ts` 的 useEventStream 即统一事件流 hook，R-23 后半在案；位置在 api/ 而非 composables/ 为形态差异，按平铺语义接受）；**补竞态守卫**——Approvals/Chat 的 onMounted await loadConn 期间卸载会留下无人 stop 的 EventSource（Feed 有守卫、另两处没有，真实泄漏），与 Feed 对齐补 disposed 守卫；**显式放弃**：① 三处 view 生命周期组装归一为公共 hook——三处模式实质不同（Feed 轮询 / Approvals 显式 start / Chat 构造即连）且各有差异化清理（confirming/retryTimer 等），是薄组件胶水（同 serve_async handler 薄层决策）；② 模块级 ref → provide/inject——单窗口 SPA 无多实例隔离需求，7 个 state 模块等效 mini-Pinia，provide/inject 引入注入链复杂度而无收益（vitest 83 全绿锁定现状） |
...
- ~~R-08 onEvent 抽离~~（✅ 2026-08-22 已完成，见 R-08 状态行）
...
- [x] （2026-08-22 按决策口径达成）命名规范：跨边界命名映射表 ✅ protocol-contract.md；**拼音乱码命名已在 R-07 清零**（复核实测：剩余 CSS 类名前缀 `tl-`/`rd-`/`pl-` 为语义缩写而非乱码——timeline/recap daily/plugins，可读性可接受，不做机械全量改名）；单字母局部变量为长期渐进项（见阶段 6 长期项说明）。
