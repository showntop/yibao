...
- **render 版本撞名覆盖（数据安全）**：`render_save` 落盘按时间线版本计数（`renders/<topic>/v<N>.mp4`），真图重渲落的 `v2.mp4` 静默覆盖了之前手工放入的 1920×1080 横版 `v2.mp4`。渲染版本应与素材版本解耦（独立递增或带内容标签），否则外部产物进同目录即被撞。模型自己在回执里报告了这起覆盖。
- **门卡批准后续跑文本不显示（真 bug，已实机复现）**：带门卡的 run 里，批准前的首段文本正常显示，批准后续跑段的 chunk 文本气泡全部空白（proc 行正常），最终回执也空白；重启重拉（从 Rust session.db）后全部正常显示——存储/渲染链路完好，问题在实时事件→内存气泡路径。单测重放同样事件序列却全部正常（useChatFlow 状态机无逻辑错），定位继续中。
