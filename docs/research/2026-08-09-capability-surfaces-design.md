...
> **2026-09-01 语义收敛：** 本文的四级表面是实现演进记录。对插件 / Capability 暴露的公共合同收敛为 `Inline / Stage / Focus` 三态；`Peek` 保留为 Host 对 Stage 的瞬态 compact placement，不拥有导航、任务或恢复点。通用对象连续性、响应式和 renderer 合同见[Agent OS 通用架构](../design/2026-09-01-agent-os-generalized-architecture.md)。
