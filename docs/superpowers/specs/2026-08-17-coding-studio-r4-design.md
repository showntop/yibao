...
3. **R4 多工位**:新面板 `coding:studio`——左栏会话列表(原会话墙)+ 中部并排工位(默认 1,经「+ 新工位」加到最多 3;验收样式收敛:空工位大片灰底显乱)+ 底部共享输入条按聚焦路由;单一自适应 UI,窄窗(面板窗)自动收成单工位、侧栏变抽屉;coding 唯一入口,chat.html 与 wall.schema.json 退役(2026-08-19 验收迭代:左栏改常驻抽屉,默认收起、☰ 常显唤出,会话区+Composer 铺满全宽)
