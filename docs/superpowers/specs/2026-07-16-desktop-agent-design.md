...
## 0.5 设计原则（贯穿 v1→v2，决策冲突时以此为准）

1. **别过早实现，但留对抽象**：协议/接口先按可扩展方向设计留口，但只实现当下要用的最小子集；让真实场景教训该补什么，别凭想象先做。（例：插件接口等做第二个垂直场景再抽象；面板协议按 schema 驱动设计，但趟路只实现 2–3 个组件。）
2. **代码 vs Agent**：确定的工作流步骤用代码/技能固化，模型只在「该判断 / 该生成」处出手。别把打分、权重、编排、判断全交给模型。
3. **先场景后抽象**：先硬做 1–2 个真实场景，从具体反推抽象，不从抽象臆想具体。
4. **底座-插件分层**：「所有场景都用」进通用底座；「只有某场景用」进垂直插件。底座不感知具体插件（单向依赖）。
5. **风险闸门贯穿**：任何操作软件的技能走 L0–L4 分级授权，高风险确认。
6. **记忆按插件隔离**：垂直插件的领域记忆用命名空间隔离，不互相污染。
