# 设计文档：本地常驻 AI 桌面 Agent 客户端（译宝 / Yibao）
...
- 每个技能/动作声明其风险等级（元数据）。
- 所有执行写入**可审计操作日志**：后端用 **SQLite**（结构化查询、原子、WAL 并发安全），截图快照存为文件、路径入库；支持回看与（尽力）撤销。
...
- **正式名**：**译宝**（Yibao）。
- **v1 默认云端 provider**：**GLM（智谱）computer-use**——国内直连、有原生 computer-use API、性价比与稳定性好；Claude computer-use 作为"最强可选"（需代理）；豆包 / Qwen-VL 作为备选。provider 走抽象层，可随时切换。
- **首批技能覆盖**：浏览器 + 文件管理器 + 系统设置 + **搜索** + **写作/文档类**（用户高频：常用软件 + 搜索 + 写东西）。
- **可撤销日志后端**：**SQLite**（截图存文件、元数据入库），v1 即内置。
