...
**同日真机补测与安全修正**：窗口裁剪样本的 100% 不代表整屏 grounding。计算器真机任务中，目标中心约 `(240,675)`，模型在 `1440×932` 整屏上连续预测到右侧 `(980–1040,350–425)`，并因 `computer_use` 默认 5 步造成一次批准后重复点击 4–5 次。修正为：GLM 原生 bbox 必须带明确 `desktop` 并通过 Quartz 匹配、裁剪目标窗口；匹配失败直接停止，不回退整屏；生产 `max_steps` 硬上限为 1，模型参数不能抬高；异步 cancel 传入技能，模型返回后、实际点击前再检查一次中断。受控评测成绩只作为窗口级定位证据，不再外推到整屏。
