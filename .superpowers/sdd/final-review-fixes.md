...
6. **panel typecheck 闸门** — package.json 加 `"typecheck": "vue-tsc --noEmit"`，devDep `vue-tsc: ^2.1.10`（解析 2.2.12，与 app 的 ^2.1.10 / typescript ^5.6 同代）。首跑暴露 1 个既存类型错误：session.test.ts:651 对 makeDeps 返回的联合类型直接取 `.mock`——trivial，已修为 `as ReturnType<typeof vi.fn>` 收窄（测试行为不变）。修后 typecheck 全绿，无遗留错误上报。
...
- `app/src-tauri cargo test`：40 passed
- `bash -n app/scripts/prepare-dist.sh` 通过；ci.yml PyYAML 解析通过
...
- app/src-tauri/target、sidecar/.venv 等未跟踪噪音未入库。
