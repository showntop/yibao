...
export const HOME_PRESETS = {
  ...
  field: {
    ...
    absent: ["now", "mind", "identity", "line", "need", "tasks", "glimpse", "catch", "scratch", "when", "spark", "sessions"],
    grid: {
      pad: 20,
      gap: 16,
      rowGap: 12,
      ground: "desk",
      fold: ["axis"], // 今日轴可折（地平线「今日」入口）；会话不常驻（§5），从「会话」入口 peek
      // 区宽呼吸，行长固定：对话保底 420px（design §8），46:27:24 分配富余
      columns: `minmax(420px, 46fr) minmax(0, 27fr) minmax(0, 24fr)`,
      rows: "minmax(0,1fr) minmax(min-content, auto)",
      areas: `"chat axis shelf" "compose . ."`, // 溪口在对话列正下方（原型如此），且折叠按列删时不伤 compose
      stacks: {
        chat: ["dayTitle", "chat"],
        axis: ["today"],
    ...
    narrow: {
      ...
      grid: {
        ...
        ground: "desk",
        fold: ["axis"],
        columns: `minmax(420px, 58fr) minmax(0, 42fr)`,
        rows: "minmax(0,1fr) minmax(min-content, auto)",
    ...
    slim: {
      ...
      grid: {
        ...
        ground: "desk",
        fold: ["axis"],
        columns: `minmax(420px, 70fr) minmax(200px, 30fr)`,
        rows: "minmax(0,1fr) minmax(min-content, auto)",
        areas: `"chat axis" "compose ."`,
        stacks: {
          chat: ["dayTitle", "chat"],
